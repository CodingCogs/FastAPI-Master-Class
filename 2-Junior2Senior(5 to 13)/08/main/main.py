...
from controller import erp_controller
...
app = FastAPI()
app.include_router(
    erp_controller.router, dependencies=[Depends(call_api_gateway)], prefix=""
)
logger.add(
    "info.log",
...
def exception_handler_student(
    ...
    return RedirectResponse(url="http://localhost:8000/student/index")
...
def exception_handler_faculty(
    ...
    return RedirectResponse(url="http://localhost:8000/faculty/index")
...
def exception_handler_library(
    ...
    return RedirectResponse(url="http://localhost:8000/library/index")
