...
def list_all_books():
    with requests.sessionion() as session:
        response = session.get("http://localhost:8001/library/book/list")
...
def list_all_issuance():
    with requests.sessionion() as session:
        response = session.get("http://localhost:8001/library/book/issuance/list")
        return response.json()
...
def return_book(returning: BookReturnReq):
    with requests.sessionion() as session:
        response = session.post(
            "http://localhost:8001/library/book/issuance/return",
